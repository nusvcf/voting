...
const VoterItem = (props: {
  ...
  return (
    <tr>
      ...
      <td>
        <div
          className={"vote-status-" + status.replace(" ", "-").toLowerCase()}
        >
          {status}
        </div>
      </td>
      <td>
        {props.vote.votedFor
          ?.map((x) => props.ballot.names.find((y) => y.id === x)?.name)
          .join(", ")}
      </td>
    </tr>
  );
...
function CandidateItem(props: { name: string; voters: string[] }) {
  return (
    <tr>
      ...
      <td>
        <ul>
          {props.voters.map((name) => (
            <li key={name} className="id">
              {name}
            </li>
          ))}
        </ul>
...
function ViewByCandidate(props: {
  ballot: Ballot;
  ...
  return (
    <table>
      <tbody>
        {props.ballot.candidateResults.map((x) => (
          <CandidateItem
            key={x.name}
            name={x.name}
            voters={x.voters.map((x) => props.lookup(x))}
          />
        ))}
      </tbody>
    </table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            ...
            {(
              (props.ballot.numVotesInBallot / props.ballot.numValidVoters) *
              100
            ).toFixed(2)}
            %)
          </td>
        ...
        <tr>
          <th>
            ...
            <br />
            <span
              style={{ fontSize: "12px", letterSpacing: "0", opacity: "0.4" }}
            >
              ({props.ballot.numDidNotVote} did not vote,{" "}
              {props.ballot.numAbstain} voted abstain)
            </span>
          </th>
          <td>
            {props.ballot.numValidVoters - props.ballot.numNonAbstainVoters} (
            {(
        ...
        <tr>
          ...
          <td>
            {props.ballot.numNonAbstainVoters} (
            {(
              (props.ballot.numNonAbstainVoters / props.ballot.numValidVoters) *
              100
            ).toFixed(2)}
            %)
          </td>
...
class ResultsModal extends Component<ResultsModalProps, { view: string }> {
  ...
  render() {
    ...
    if (this.state.view === "voter") {
      ...
      view = (
        <ViewByCandidate ballot={this.props.ballot} lookup={this.lookup} />
      );
    }
