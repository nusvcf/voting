		...
		XlsxPopulate.fromFileAsync('./Votes.xlsx', {
			...
			Object.keys(ballot.submittedUsers).forEach((id) => {
				workbook
					.sheet(ballot.position + ballotsLength)
					.range('A' + i + ':C' + i)
					.value([ [ id, ballot.submittedUsers[id].votedFor.join(', '), ballot.submittedUsers[id].status ] ]);
				i++;
			});
			// Write to file.
	...
	.get(checkIsAdmin, (req, res) => {
		...
		for (let i = 0; i < numBallots; i++) {
			...
			if (ballot.isOpen) {
				...
				if (numVoters !== 0) ballotPercentVotes = numVotesInBallot / numVoters * 100;
			} else {
			...
			for (let i = 0; i < length; i++) {
				...
				if (numVotesInBallot !== 0) namePercentageVotes = count / numVotesInBallot * 100;
